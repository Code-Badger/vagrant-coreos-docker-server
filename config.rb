...
# 外部ストレージ
$attached_storages = [
  {:file => 'data.vdi', :size => 100*1024, :port => 1, :device => 0, :dev => 'sdb', :dest => '/mnt'},
  {:file => 'image.vdi', :size => 100*1024, :port => 1, :device => 1, :dev => 'sdc', :dest => '/var/lib'}
]
...
$initialize = <<-EOF
  ...
  sudo mkdir -p /mnt/data/resourcespace/filestore 2>/dev/null
  sudo mkdir -p /mnt/data/resourcespace/homeanim 2>/dev/null
  sudo mkdir -p /mnt/data/resourcespace/homeanim/gfx 2>/dev/null
  sudo chmod 767 -R /mnt/data/resourcespace/*
EOF
