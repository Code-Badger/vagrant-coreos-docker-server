...
Vagrant.configure("2") do |config|
  ...
  (1..$num_instances).each do |i|
    config.vm.define vm_name = "%s-%02d" % [$instance_name_prefix, i] do |config|
      ...
      # comment out to use public_network
      ...
      # format and mount storage if it isn't exist
      # if not File.exist?($attached_storages[0][:file]) then
        init_storage = format_storage($attached_storages[0][:dev], $attached_storages[0][:dest])
        config.vm.provision :shell, :inline => init_storage, :privileged => true
      # end
      # if not File.exist?($attached_storages[1][:file]) then
        init_storage = format_storage($attached_storages[1][:dev], $attached_storages[1][:dest])
        config.vm.provision :shell, :inline => init_storage, :privileged => true
      # end
      ...
      config.vm.provider :virtualbox do |vb|
        for data in $attached_storages
          ...
          # attach to vm
          vb.customize ['storageattach', :id, '--storagectl', 'IDE Controller', '--port', data[:port], '--device', data[:device], '--type', 'hdd', '--medium', data[:file]]
        end
      ...
      config.vm.provision :shell, :inline => $initialize, :privileged => true
